...
if(isset($_POST['submit'])){
   ...
   // Récupérer et filtrer le nom, l'email et les mots de passe du formulaire
   $name = $_POST['name'];
   ...
   // Récupérer et filtrer l'image téléchargée
   ...
   if($select_user->rowCount() > 0){
      ...
      if($pass != $cpass){
         ...
         // Insérer l'utilisateur dans la base de données et télécharger l'image
         $insert_user = $conn->prepare("INSERT INTO `users`(id, name, email, password, image) VALUES(?,?,?,?,?)");
         $insert_user->execute([$id, $name, $email, $cpass, $rename]);
         move_uploaded_file($image_tmp_name, $image_folder);
         
         // Vérifier l'utilisateur et le connecter
