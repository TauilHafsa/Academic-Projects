...
if(isset($_POST['submit'])){
   ...
   // Requête pour sélectionner le tuteur en fonction de l'email et du mot de passe
   $select_tutor = $conn->prepare("SELECT * FROM `tutors` WHERE email = ? AND password = ? LIMIT 1");
   $select_tutor->execute([$email, $pass]);
   $row = $select_tutor->fetch(PDO::FETCH_ASSOC);
   
   // Si un tuteur correspondant est trouvé, créer un cookie pour l'identifiant du tuteur et rediriger vers le tableau de bord
